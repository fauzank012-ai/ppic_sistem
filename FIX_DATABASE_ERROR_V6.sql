...
-- This script updates the report_view_mat VIEW to only take the latest stock data using 'created_at'.
...
-- Fix: Only take the latest stock data using 'created_at'
stock_agg AS (
    SELECT 
    ...
    FROM public.stocks
    WHERE created_at = (SELECT MAX(created_at) FROM public.stocks)
