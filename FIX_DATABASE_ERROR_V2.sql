-- Aggressive Fix for the "invalid value 'Mare' for 'YYYY'" error
-- This script drops all dependent objects before altering column types.
...
-- 2. Drop Views and Materialized Views that depend on 'periode' or 'periode_bulan'
DROP MATERIALIZED VIEW IF EXISTS public.report_view_mat CASCADE;
DROP VIEW IF EXISTS public.report_view CASCADE;
DROP VIEW IF EXISTS public.mapped_deliveries CASCADE;
DROP VIEW IF EXISTS public.dashboard_forecast_vs_actual CASCADE;
...
-- 4. Recreate mapped_deliveries view
...
-- 5. Recreate the Materialized View
...
CREATE TRIGGER refresh_report_on_mm_change AFTER INSERT OR UPDATE OR DELETE ON public.material_master FOR EACH STATEMENT EXECUTE FUNCTION refresh_report_view();
CREATE TRIGGER refresh_report_on_loo_change AFTER INSERT OR UPDATE OR DELETE ON public.loo_data FOR EACH STATEMENT EXECUTE FUNCTION refresh_report_view();
CREATE TRIGGER refresh_report_on_so_change AFTER INSERT OR UPDATE OR DELETE ON public.sales_orders FOR EACH STATEMENT EXECUTE FUNCTION refresh_report_view();
CREATE TRIGGER refresh_report_on_stock_change AFTER INSERT OR UPDATE OR DELETE ON public.stocks FOR EACH STATEMENT EXECUTE FUNCTION refresh_report_view();
CREATE TRIGGER refresh_report_on_delivery_change AFTER INSERT OR UPDATE OR DELETE ON public.deliveries FOR EACH STATEMENT EXECUTE FUNCTION refresh_report_view();
CREATE TRIGGER refresh_report_on_forecast_change AFTER INSERT OR UPDATE OR DELETE ON public.forecasts FOR EACH STATEMENT EXECUTE FUNCTION refresh_report_view();
