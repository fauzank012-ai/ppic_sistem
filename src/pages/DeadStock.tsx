...
export default function DeadStock() {
  ...
  const [deadMaterialCodes, setDeadMaterialCodes] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [stocksData, materialsData, looData, soData, forecastData] = await Promise.all([
          fetchAllRows('stocks', 'kode_material,wip_lt_pcs,wip_st_pcs,fg_st_pcs,fg_lt_pcs,wip_st_kg,wip_lt_kg,fg_st_kg,fg_lt_kg,grade,created_at,jenis_stock,lokasi_gudang').catch(() => []),
          fetchAllRows('material_master', 'kode_st,kode_lt,kode_strip,alternative_kodes_st,alternative_kodes_lt,alternative_kode_strip,berat_per_pcs,dimensi,konversi_lt_ke_st').catch(() => []),
          fetchAllRows('loo_data', 'kode_st,kode_lt,sisa_loo_pcs,sisa_order_pcs,sisa_loo_kg,sisa_order_kg').catch(() => []),
          fetchAllRows('sales_orders', 'kode_st,kode_lt,qty_order_pcs,qty_order_kg').catch(() => []),
          fetchAllRows('forecasts', 'kode_st,kode_lt,qty_pcs,qty_forecast_kg').catch(() => [])
        ]);
        ...
        const uniqueStockMaterials = new Set((stocksData || []).map(s => (s.kode_material || '').trim().toLowerCase()));

        uniqueStockMaterials.forEach(kodeMat => {
          ...
          if (!hasReq) {
            deadCodes.add(kodeMat);
          }
        });

        setDeadMaterialCodes(deadCodes);
  ...
  const uniqueDeadMaterials = useMemo(() => {
    // Get materials that are DEAD in the latest stock data
    const deadMaterialsMap = new Map<string, string>(); // Map kode -> jenis_stock
    ...
    if (latestDate) {
      ...
      stocks.forEach(s => {
        ...
        if (d.getDate() === targetDay && 
          ...
          if (deadMaterialCodes.has(kodeMat)) {
            const totalPcs = (s.wip_lt_pcs || 0) + (s.wip_st_pcs || 0) + (s.fg_st_pcs || 0) + (s.fg_lt_pcs || 0);
            if (totalPcs > 0) {
              deadMaterialsMap.set(kodeMat, s.jenis_stock || '-');
            }
          }
        }
      });
    ...
    return Array.from(deadMaterialsMap.entries()).map(([kode, jenis]) => {
  ...
  }, [stocks, materialMap, deadMaterialCodes]);
  ...
  const chartData = useMemo(() => {
    ...
    stocks.filter(s => {
      ...
      return deadMaterialCodes.has(kodeMat);
    }).forEach(item => {
  ...
  const filteredAndSortedData = useMemo(() => {
    let result = [...uniqueDeadMaterials];
    ...
    return result;
  }, [uniqueDeadMaterials, searchTerm, sortConfig]);
  ...
  const handleExport = async () => {
    ...
    xlsx.writeFile(wb, 'Dead_Stock.xlsx');
  };
            ...
            <div className="h-[250px]">
              {chartData.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-full text-gray-500">
                  Tidak ada data Dead Stock untuk ditampilkan grafiknya.
                </div>
              )}
            </div>
          </div>
          ...
          <>
              ...
              <table className="w-full text-left border-collapse text-[11px]">
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={dateColumns.length + 4} className="px-2 py-8 text-center text-gray-500">
                        Tidak ada data ditemukan untuk Dead Stock
                      </td>
                    </tr>
                  ) : (
            ...
            <div className="p-3 border-t bg-gray-50 text-[10px] text-gray-500 flex justify-between items-center">
              <div className="flex items-center gap-4">
                <span>Menampilkan {paginatedData.length} dari {filteredAndSortedData.length} material Dead Stock</span>
