...
export default function ExcessStock() {
  ...
  const [excessMaterialCodes, setExcessMaterialCodes] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [stocksData, materialsData, looData, soData, forecastData, deliveryData] = await Promise.all([
          fetchAllRows('stocks', 'kode_material,wip_lt_pcs,wip_st_pcs,fg_st_pcs,fg_lt_pcs,wip_st_kg,wip_lt_kg,fg_st_kg,fg_lt_kg,jenis_stock,lokasi_gudang,created_at'),
          fetchAllRows('material_master', 'kode_st,kode_lt,kode_strip,alternative_kodes_st,alternative_kodes_lt,alternative_kode_strip,berat_per_pcs,dimensi,konversi_lt_ke_st'),
        ...
        materialsData.forEach(m => {
          ...
          if (m.alternative_kodes_st) {
            m.alternative_kodes_st.split(',').forEach(addMat);
          }
          if (m.alternative_kodes_lt) {
            m.alternative_kodes_lt.split(',').forEach(addMat);
          }
          if (m.alternative_kode_strip) {
            m.alternative_kode_strip.split(',').forEach(addMat);
          }
        });
        ...
        const deliveryMap = new Map<string, { total: number, days: Set<string> }>();
        (deliveryData || []).forEach(d => {
          const kode = (d.kode_st || '').trim().toLowerCase();
          if (!kode) return;
          if (!deliveryMap.has(kode)) {
            deliveryMap.set(kode, { total: 0, days: new Set() });
          }
          const entry = deliveryMap.get(kode)!;
          entry.total += (d.qty_delivery_pcs || 0);
          if (d.tanggal_delivery) {
            entry.days.add(d.tanggal_delivery.split('T')[0]);
          }
        });
        ...
        const getAvgDelivery = (kode: string) => {
          ...
          return entry.total / uniqueDays;
        };

        const excessCodes = new Set<string>();
        
        const latestStockMap = new Map<string, any>();
        (stocksData || []).forEach(s => {
          const kodeMat = (s.kode_material || '').trim().toLowerCase();
          if (!latestStockMap.has(kodeMat)) {
            latestStockMap.set(kodeMat, s);
          } else {
            const current = latestStockMap.get(kodeMat);
            if (new Date(s.created_at) > new Date(current.created_at)) {
              latestStockMap.set(kodeMat, s);
            }
          }
        });

        latestStockMap.forEach((s, kodeMat) => {
          ...
          if (!hasReq) {
            for (const mat of relatedMats) {
              ...
              if (checkReq(mat.kode_st) || checkReq(mat.kode_lt) || checkReq(mat.kode_strip)) {
                hasReq = true;
                break;
              }
              
              if (mat.alternative_kodes_st && mat.alternative_kodes_st.split(',').some(checkReq)) {
                hasReq = true;
                break;
              }
              if (mat.alternative_kodes_lt && mat.alternative_kodes_lt.split(',').some(checkReq)) {
                hasReq = true;
                break;
              }
              if (mat.alternative_kode_strip && mat.alternative_kode_strip.split(',').some(checkReq)) {
                hasReq = true;
                break;
              }
            }
          ...
          if (hasReq) {
            const mat = relatedMats.length > 0 ? relatedMats[0] : null;
            const konversi = mat ? (mat.konversi_lt_ke_st || 0) : 0;
            const totalStock = (s.wip_st_pcs || 0) + (s.fg_st_pcs || 0) + (((s.wip_lt_pcs || 0) + (s.fg_lt_pcs || 0)) * konversi);
              ...
              relatedMats.forEach(m => {
                ...
                if (m.alternative_kodes_st) {
                  m.alternative_kodes_st.split(',').forEach((alt: string) => {
                    if (alt.trim()) uniqueSTs.add(alt.trim().toLowerCase());
                  });
                }
              });
              
              let totalRelatedDelivery = 0;
              uniqueSTs.forEach(st => {
                totalRelatedDelivery += getAvgDelivery(st);
              });
            ...
            let doc = 0;
            if (avgDelivery > 0) {
              doc = totalStock / avgDelivery;
            } else if (totalStock > 0) {
              doc = Infinity;
            }

            if (doc > 30) {
              excessCodes.add(kodeMat);
            }
          }
        });

        setExcessMaterialCodes(excessCodes);
  ...
  const uniqueExcessMaterials = useMemo(() => {
    // Get materials that are EXCESS in the latest stock data
    const excessMaterialsMap = new Map<string, string>(); // Map kode -> jenis_stock
    ...
    if (latestDate) {
      ...
      stocks.forEach(s => {
        ...
        if (d.getDate() === targetDay && 
          ...
          const kodeMat = normalize(s.kode_material);
          if (excessMaterialCodes.has(kodeMat)) {
            const totalKg = (s.wip_st_kg || 0) + (s.fg_st_kg || 0) + (s.wip_lt_kg || 0) + (s.fg_lt_kg || 0);
            if (totalKg > 0) {
              excessMaterialsMap.set(kodeMat, s.jenis_stock || '-');
            }
          }
        }
      });
    ...
    return Array.from(excessMaterialsMap.entries()).map(([kode, jenis]) => {
  ...
  }, [stocks, materialMap, excessMaterialCodes]);
  ...
  const chartData = useMemo(() => {
    ...
    stocks.filter(s => {
      const kodeMat = (s.kode_material || '').trim().toLowerCase();
      return excessMaterialCodes.has(kodeMat);
    }).forEach(item => {
  ...
  const summaryChartsData = useMemo(() => {
    ...
    stocks.forEach(s => {
      ...
      if (d.getDate() === targetDay && 
        ...
        const kodeMat = normalize(s.kode_material);
        if (excessMaterialCodes.has(kodeMat)) {
          const info = materialMap.get(kodeMat);
  ...
  const filteredAndSortedData = useMemo(() => {
    let result = [...uniqueExcessMaterials];
    ...
    return result;
  }, [uniqueExcessMaterials, searchTerm, sortConfig]);
  ...
  const handleExport = async () => {
    ...
    const exportData = filteredAndSortedData.map(row => {
      ...
      dateColumns.forEach(date => {
        ...
        const sameDayStocks = (stocksGrouped.get(key) || []).filter(s => {
          const kodeMat = (s.kode_material || '').trim().toLowerCase();
          return excessMaterialCodes.has(kodeMat);
        });
    ...
    xlsx.writeFile(wb, 'Excess_Stock.xlsx');
  };
            ...
            <div className="h-[250px]">
              {chartData.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-full text-gray-500">
                  Tidak ada data Excess Stock untuk ditampilkan grafiknya.
                </div>
              )}
            </div>
          </div>
          ...
          <>
              ...
              <table className="w-full text-left border-collapse text-[11px]">
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={dateColumns.length + 4} className="px-2 py-8 text-center text-gray-500">
                        Tidak ada data ditemukan untuk Excess Stock
                      </td>
                    </tr>
                  ) : (
                    ...
                    <tr className="bg-teal-700 text-white font-bold">
                      ...
                      {dateColumns.map(date => {
                        ...
                        const totalForDay = filteredAndSortedData.reduce((sum, row) => {
                          ...
                          if (sameDayStocks.length > 0) {
                            const totalPcsST = sameDayStocks.reduce((acc, s) => 
                              acc + (s.wip_st_pcs || 0) + (s.fg_st_pcs || 0) + (((s.wip_lt_pcs || 0) + (s.fg_lt_pcs || 0)) * row.konversi), 0
                            );
                            return sum + (totalPcsST * row.weight);
                          }
                          return sum;
                        }, 0);
            ...
            <div className="p-3 border-t bg-gray-50 text-[10px] text-gray-500 flex justify-between items-center">
              <div className="flex items-center gap-4">
                <span>Menampilkan {paginatedData.length} dari {filteredAndSortedData.length} material Excess Stock</span>
